            ...
            <ButtonToolbar style={this.props.style} data-read-only={(this.props.disabled ? 1 : 0)}>                        
                <ToggleButtonGroup size={this.props.bsSize} type={this.props.type} name={this.props.name} defaultValue={this.props.defaultValue} onChange={this.onChange}>                                
                    {this.props.options.map((item, index) => {   
                        let element = 
                            <BsToggleButton className='rounded' key={index} variant={(this.props.defaultValue.includes(item.value) ? "primary" : "light")} onClick={(e) => this.onClick(item.value, e)} value={item.value} disabled={this.props.disabled}>
                                {item.text}
                            </BsToggleButton>;
                        return (element);
                    })}                                    
